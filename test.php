                    ...
                    <select class="form-control name_email" name="item_list[]" >
                              <?php
                                ...
                                while($subData=sqlsrv_fetch_array($subQuery)){
                                  $dats=$subData['ID'];
                              ?>    
                                    <option value="<?php echo (string)$dats;?>"><?php echo "IT00".$dats." -- ".$subData['ItemName']; }?></option>
                                </select>
                </td>
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    $("#add").click(function(){
        i++;
    $('#dynamic_field').append('<tr id="row'+i+'"><td><select name="" class="form-control name_email"><option value="dad">dada</option></select></td><td><input type="text" name="email[]" placeholder="Enter your Email" class="form-control name_email"/></td><td><button type="button" name="remove" id="'+i+'" class="btn btn-danger btn_remove">X</button></td></tr>');  
    });
